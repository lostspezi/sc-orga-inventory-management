...
import { saveSocialSettingsAction } from "@/lib/actions/save-social-settings-action";
...
export default function SocialSettingsForm({ settings }: { settings: SocialSettingsView }) {
    const [state, action, pending] = useActionState(saveSocialSettingsAction, null);

    return (
        <form action={action} className="space-y-6">
            {PLATFORMS.map(({ key, label, placeholder, color, icon }) => (
                <div key={key}>
                    <label
                        htmlFor={key}
                        className="mb-2 flex items-center gap-2 text-[10px] uppercase tracking-[0.2em]"
                        style={{ color: `${color},0.6)`, fontFamily: "var(--font-mono)" }}
                    >
                        <span style={{ color: `${color},0.7)` }}>{icon}</span>
                        {label}
                    </label>
                    <input
                        id={key}
                        name={key}
                        type="url"
                        defaultValue={settings[key as PlatformKey]}
            ...
            {state?.success && (
                <p className="text-xs" style={{ color: "rgba(80,210,120,0.85)", fontFamily: "var(--font-mono)" }}>
                    Social links saved.
                </p>
            )}
            ...
            <div className="flex justify-end">
                <button
                    type="submit"
                    disabled={pending}
                    className="sc-btn sc-btn-outline px-6 py-2 text-xs disabled:opacity-50"
                >
